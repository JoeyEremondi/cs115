...
- [[#multisets-with-linked-lists][Multisets with Linked Lists]]
...
** Linked List application: Multiset Revisited
 
*** Recall: Multisets
- Datatype for storing 1 /or more/ copies of some elements
  + Keep count of how many of each item are in
  + Removing only removes one copy of an item

*** ADT Multiset without Upper Bounds
...
public:
  ...
  // Assignment operator
  Multiset
    &operator=(const Multiset &original);
  // Other functions
  bool empty() const;
  unsigned int memberCount() const;
  void insert(ItemType x);
